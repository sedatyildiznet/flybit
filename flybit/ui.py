...
class BrainWorker(QObject):
    ...
    def start(self) -> None:
        try:
            ...
            self.ready.emit(
                self._core.device,
                self._core.neuron_count,
                self._core.graded_cell_count,
            )
    ...
    def _step(self) -> None:
        ...
        try:
            self.snapshot.emit(
                self._core.step_visual_target(self._target)
            )
        except Exception as exc:
            if self._timer:
                self._timer.stop()
...
class FlyGlyph(QWidget):
    """Neutral visual marker; it does not select or animate behaviour."""
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        painter.setPen(
            QPen(QColor(205, 205, 205), 1.2)
        )
        painter.setBrush(QBrush(QColor(30, 30, 30)))
        painter.drawEllipse(26, 16, 18, 24)
        painter.setBrush(
            QBrush(QColor(170, 170, 170, 80))
        )
...
class FlybitWindow(QWidget):
    ...
    def __init__(self) -> None:
        ...
        self.metrics = QLabel(
            "First launch may download ~260 MB of connectome data."
        )
        ...
        self.note = QLabel(
            "R1–R8 + L1–L3 graded · no feature shortcut · no scripted behaviour"
        )
        ...
        self._input_timer.timeout.connect(
            self._sample_cursor
        )
    ...
    def paintEvent(self, event) -> None:  # noqa: N802
        ...
        painter.setPen(
            QPen(QColor(70, 70, 70, 210), 1)
        )
        painter.setBrush(
            QBrush(QColor(16, 16, 16, 225))
        )
    ...
    def _on_ready(
        ...
        self.status.setText(
            f"MaleCNS online · {device.upper()}"
        )
        self.metrics.setText(
            f"neurons {neurons:,}   graded visual {graded_cells:,}\n"
            "waiting for retinal contrast…"
        )
    ...
    def _on_snapshot(
        ...
        self.metrics.setText(
            ...
            f"visual projection spikes {snap.visual_projection_spikes:,}\n"
            f"descending spikes {snap.descending_spikes:,}   "
            f"active {snap.descending_active:,}"
        )
